...
function getToken() { return window._nexusToken || ''; }
function setToken(t) { window._nexusToken = t; }
function setUsuario(u) { window._nexusUsuario = u; }
...
const S = {
  page: { fontFamily: "'DM Sans',system-ui,sans-serif", background: '#F5F4F0', minHeight: '100vh', color: '#1C1B19' },
  card: { background: '#FAFAF8', border: '0.5px solid #DDD9D3', borderRadius: 12, padding: '1.25rem' },
  input: { width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #D4D1CA', fontFamily: 'inherit', fontSize: 14, boxSizing: 'border-box', background: '#fff', outline: 'none' },
  btnPrimary: { background: '#185FA5', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' },
  btnSecondary: { background: '#EDEAE5', color: '#1C1B19', border: '1px solid #D4D1CA', borderRadius: 8, padding: '10px 20px', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' },
  btnDanger: { background: '#F5ECEB', color: '#A32D2D', border: '1px solid #ECC', borderRadius: 8, padding: '10px 20px', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' },
  label: { display: 'block', marginBottom: 5, fontSize: 12, fontWeight: 500, color: '#7A7974' },
  ...
  tabBtn: (active) => ({
    padding: '8px 16px', border: 'none', borderRadius: 6, cursor: 'pointer', fontFamily: 'inherit',
...
function Spinner() {
  return <span style={{ display: 'inline-block', width: 16, height: 16, border: '2px solid #ccc', borderTopColor: '#185FA5', borderRadius: '50%', animation: 'spin 0.7s linear infinite', marginRight: 6 }} />;
}
...
function SuccessCard({ numeroCuenta, onGoLogin }) {
  return (
    <div style={S.card}>
      ...
      <p style={{ margin: '0 0 1rem', fontSize: 12, color: '#7A7974' }}>Guárdalo, lo usarás para recibir transferencias. Ahora ya puedes iniciar sesión.</p>
      <button style={{ ...S.btnPrimary, width: '100%' }} onClick={onGoLogin}>Ir a iniciar sesión</button>
    </div>
  );
}
...
function TxRow({ tx }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '0.5px solid #E8E5E0' }}>
      <div style={{ width: 32, height: 32, borderRadius: '50%', background: t.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, color: t.color, flexShrink: 0 }}>{t.icon}</div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ margin: 0, fontSize: 13, fontWeight: 500, color: '#1C1B19', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{tx.descripcion}</p>
        <p style={{ margin: 0, fontSize: 11, color: '#AAA8A3' }}>{t.label} · {formatDateTime(tx.fecha)}</p>
      </div>
      <div style={{ textAlign: 'right', flexShrink: 0 }}>
        <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: isPositive ? '#3B6D11' : '#A32D2D' }}>{isPositive ? '+' : '−'}{formatCurrency(tx.monto)}</p>
    ...
    <div style={{ ...S.page, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 1rem' }}>
      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '2rem', justifyContent: 'center' }}>
          <div style={{ width: 42, height: 42, borderRadius: 10, background: '#185FA5', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#E6F1FB', fontSize: 20, fontFamily: 'Georgia,serif' }}>N</div>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 400, fontFamily: "'DM Serif Display',Georgia,serif", color: '#1C1B19' }}>Banco Nexus</h1>
        </div>

        {successAccount ? (
          <SuccessCard numeroCuenta={successAccount} onGoLogin={() => { setMode('login'); setSuccessAccount(''); setError(''); }} />
        ) : (
          <div style={S.card}>
...
function Dashboard({ usuario, onLogout }) {
  ...
  useEffect(() => {
    ...
    src.addEventListener('db-alert', (e) => {
      try { setDbAlert(JSON.parse(e.data)); } catch (_) {}
    });
    ...
    <div style={{ ...S.page, padding: 0 }}>
      ...
      <header style={{ background: '#185FA5', color: '#fff', padding: '0 1.5rem', display: 'flex', alignItems: 'center', gap: 16, height: 56, position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
          <div style={{ width: 32, height: 32, borderRadius: 7, background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#185FA5', fontFamily: 'Georgia,serif', fontWeight: 700, fontSize: 16 }}>N</div>
        ...
        <button onClick={onLogout} style={{ background: 'rgba(255,255,255,0.15)', border: 'none', borderRadius: 6, color: '#fff', padding: '5px 12px', fontSize: 12, cursor: 'pointer' }}>Salir</button>
      </header>
      <nav style={{ background: '#fff', borderBottom: '0.5px solid #DDD9D3', padding: '0 1rem', display: 'flex', gap: 4, overflowX: 'auto' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            padding: '12px 14px', border: 'none', background: 'none', cursor: 'pointer', fontFamily: 'inherit',
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '1.5rem 1rem' }}>
        ...
        {tab === 'transferir' && <TabTransferir cuentasDestino={cuentasDestino} onSuccess={() => { cargarCuenta(); showStatus('success', 'Transferencia realizada correctamente.'); }} onError={(e) => showStatus('error', e)} />}
...
function TabInicio({ cuenta, historial, usuario }) {
  return (
    <div>
      <h2 style={{ marginTop: 0, fontFamily: "'DM Serif Display',Georgia,serif", fontWeight: 400, fontSize: 22 }}>Bienvenido, {usuario.nombre}</h2>
      <div style={{ background: '#185FA5', borderRadius: 14, padding: '1.5rem', color: '#fff', marginBottom: '1.5rem' }}>
        ...
        <p style={{ margin: '0 0 12px', fontSize: 32, fontFamily: "'DM Serif Display',Georgia,serif", fontWeight: 400 }}>{cuenta ? formatCurrency(cuenta.saldo) : '—'}</p>
      ...
      <div style={S.card}>
        ...
        {historial.length === 0 ? <p style={{ color: '#AAA', fontSize: 13, textAlign: 'center', padding: '2rem 0' }}>Sin movimientos aún</p> : historial.slice(0, 5).map((tx, i) => <TxRow key={i} tx={tx} />)}
      </div>
    </div>
  );
}
...
function TabHistorial({ historial }) {
  return (
    <div style={S.card}>
      ...
      {historial.length === 0 ? <p style={{ color: '#AAA', fontSize: 13, textAlign: 'center', padding: '2rem 0' }}>No hay movimientos</p> : historial.map((tx, i) => <TxRow key={i} tx={tx} />)}
    </div>
  );
}
  ...
  async function handleConfirm() {
    ...
    try {
      await apiFetch('/api/transferencia', {
        method: 'POST',
        body: JSON.stringify({ cuentaDestino: form.cuentaDestino, monto: Number(form.monto), mensaje: form.mensaje })
      });
          ...
          {cuentasDestino.length > 0 && (
            <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              ...
              {cuentasDestino.map(cd => (
                <button key={cd._id} type="button" onClick={() => setForm(f => ({ ...f, cuentaDestino: cd.numeroCuenta }))} style={{ ...S.btnSecondary, padding: '3px 10px', fontSize: 11 }}>{cd.alias}</button>
              ))}
            </div>
          )}
        </div>
        ...
        {confirmOpen && (
          <div style={{ background: '#F0F6FF', border: '1px solid #C8DCF5', borderRadius: 10, padding: '12px', marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: '12px' }}>
              ...
              <button type="button" onClick={handleConfirm} style={{ ...S.btnPrimary, flex: 1 }} disabled={loading}>{loading ? 'Procesando...' : 'Confirmar'}</button>
            </div>
          </div>
        )}
        {!confirmOpen && <button style={{ ...S.btnPrimary, width: '100%', padding: 12 }} type="submit" disabled={loading}>{loading ? <><Spinner />Procesando...</> : '↔ Transferir'}</button>}
      </form>
    </div>
  );
}
...
function TabDestinos({ cuentasDestino, onRefresh, onStatus }) {
  ...
  async function handleAgregar(e) {
    ...
    setLoading(true);
    try {
      await apiFetch('/api/cuentas-destino', { method: 'POST', body: JSON.stringify(form) });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <div style={S.card}>
        ...
        {cuentasDestino.length === 0 ? <p style={{ color: '#AAA', fontSize: 13, textAlign: 'center', padding: '2rem 0' }}>No tienes cuentas guardadas aún</p> : cuentasDestino.map(cd => (
          <div key={cd._id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '0.5px solid #E8E5E0' }}>
            ...
            <button onClick={() => handleEliminar(cd._id)} style={{ ...S.btnDanger, padding: '5px 12px', fontSize: 12 }}>Eliminar</button>
          </div>
        ))}
      </div>
    </div>
...
function TabPerfil({ usuario, cuenta, onStatus }) {
  const [form, setForm] = useState({ nombre: usuario.nombre || '', apellidoPaterno: usuario.apellidoPaterno || '', telefono: usuario.telefono || '' });
  const [loading, setLoading] = useState(false);
  const set = (k) => (e) => setForm(f => ({ ...f, [k]: e.target.value }));
  ...
  useEffect(() => {
    async function cargarPerfil() {
      try {
        ...
        const cliente = data.cliente || data.usuario || data.perfil || {};
        setForm({
          nombre: cliente.nombre || usuario.nombre || '',
          apellidoPaterno: cliente.apellidoPaterno || usuario.apellidoPaterno || '',
          telefono: cliente.telefono || usuario.telefono || ''
        });
      } catch (_) {}
    }
    cargarPerfil();
  ...
  async function handleActualizar(e) {
    ...
    setLoading(true);
    try {
      await apiFetch('/api/perfil', { method: 'PUT', body: JSON.stringify(form) });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div style={{ ...S.card, background: '#F0F6FF', border: '1px solid #C8DCF5' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: 13 }}>
          <div><span style={{ color: '#888' }}>Número de cuenta</span><p style={{ margin: '2px 0 0', fontFamily: 'monospace', fontWeight: 600 }}>{usuario.numeroCuenta}</p></div>
          <div><span style={{ color: '#888' }}>Correo</span><p style={{ margin: '2px 0 0' }}>{usuario.correo}</p></div>
          <div><span style={{ color: '#888' }}>Tipo</span><p style={{ margin: '2px 0 0', textTransform: 'capitalize' }}>{cuenta?.tipoCuenta || '—'}</p></div>
          <div><span style={{ color: '#888' }}>Estatus</span><p style={{ margin: '2px 0 0', color: '#3B6D11', fontWeight: 500 }}>{cuenta?.estatus || '—'}</p></div>
        </div>
      </div>
      <div style={S.card}>
        ...
        <form onSubmit={handleActualizar}>
          ...
          <button style={S.btnPrimary} type="submit" disabled={loading}>{loading ? 'Guardando...' : 'Guardar cambios'}</button>
        </form>
      </div>
    </div>
  );
...
export default function App() {
  ...
  function handleAuth(u) { setUsuarioState(u); }
  function handleLogout() { setToken(''); setUsuario(null); setUsuarioState(null); }
