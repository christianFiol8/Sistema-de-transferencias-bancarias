...
function getUsuario() { return window._nexusUsuario || null; }
...
// ─── Estilos base inline (sin CSS externo) ────────────────────────────────
const S = {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (mode === 'login') {
        ...
        setError('');
        alert(`✅ Cuenta creada. Tu número de cuenta es: ${data.numeroCuenta}\nAhora inicia sesión.`);
        setMode('login');
        setForm({});
      }
    } catch (err) {
...
function Dashboard({ usuario, onLogout }) {
  ...
  // SSE — actualización en tiempo real de cuenta
  useEffect(() => {
  ...
  return (
    <div style={{ ...S.page, padding: 0 }}>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '1.5rem 1rem' }}>
        ...
        {tab === 'transferir' && <TabTransferir usuario={usuario} cuentasDestino={cuentasDestino} onSuccess={() => { cargarCuenta(); showStatus('success', 'Transferencia realizada correctamente.'); }} onError={(e) => showStatus('error', e)} />}
...
// ─── Tab: Transferir ─────────────────────────────────────────────────────
function TabTransferir({ usuario, cuentasDestino, onSuccess, onError }) {
...
function TabPerfil({ usuario, cuenta, onStatus }) {
  const [form, setForm]   = useState({ nombre: usuario.nombre || '', apellidoPaterno: usuario.apellidoPaterno || '', telefono: '' });
  const [loading, setLoading] = useState(false);
  const set = (k) => (e) => setForm(f => ({ ...f, [k]: e.target.value }));
...
export default function App() {
  ...
  function handleAuth(u) { setUsuario(u); }
  function handleLogout() { setToken(''); setUsuario(null); }
